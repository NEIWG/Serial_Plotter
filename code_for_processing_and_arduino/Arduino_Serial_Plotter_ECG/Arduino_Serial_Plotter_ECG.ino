...
void loop()
{
  ...
  if (ads1292dataReceived == true)      // process the data
  {
    ...
    //OY 7/31/2018 print ECG output
    //Serial.println(ecg_wave_buff[0]);
    //OY 8/7/2018 print a 24 bit number 
    long usec = micros() - old_usec;
    old_usec = micros();
    ///////Serial.print(micros());Serial.print(",");
    Serial.println(s32DaqVals[1]);
    //Serial.print(usec);Serial.print(",");Serial.println(s32DaqVals[1]);

    //Serial.println(";");
    //byte b=byte(abs(ecg_wave_buff[0])/4);
    //Serial.write(b);
